...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::CaritasCoin)
    {
    }
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    // Combined balances
    CAmount availableTotalBalance = CaritasCoinAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // CaritasCoin labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, CaritasCoinAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    if (fEnableZeromint) {
        ...
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in caritas.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in caritas.conf");
    }
    ...
    bool showCaritasCoinAvailable = settingShowAllBalances || CaritasCoinAvailableBalance != nTotalBalance;
    bool showWatchOnlyVITAvailable = watchOnlyBalance != nTotalWatchBalance;
    bool showCaritasCoinPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlyVITPending = watchUnconfBalance != 0;
    bool showCaritasCoinLocked = settingShowAllBalances || nLockedBalance != 0;
    ...
    ui->labelBalance->setVisible(showCaritasCoinAvailable || showWatchOnlyVITAvailable);
    ui->labelBalanceText->setVisible(showCaritasCoinAvailable || showWatchOnlyVITAvailable);
    ui->labelWatchAvailable->setVisible(showCaritasCoinAvailable && showWatchOnly);
    ui->labelUnconfirmed->setVisible(showCaritasCoinPending || showWatchOnlyVITPending);
    ui->labelPendingText->setVisible(showCaritasCoinPending || showWatchOnlyVITPending);
    ui->labelWatchPending->setVisible(showCaritasCoinPending && showWatchOnly);
    ui->labelLockedBalance->setVisible(showCaritasCoinLocked || showWatchOnlyVITLocked);
    ui->labelLockedBalanceText->setVisible(showCaritasCoinLocked || showWatchOnlyVITLocked);
    ui->labelWatchLocked->setVisible(showCaritasCoinLocked && showWatchOnly);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("CaritasCoin")
    updateDisplayUnit();
}
