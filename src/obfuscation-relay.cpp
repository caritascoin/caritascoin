...
CObfuScationRelay::CObfuScationRelay(CTxIn& vinCoralnodeIn, vector<unsigned char>& vchSigIn, int nBlockHeightIn, int nRelayTypeIn, CTxIn& in2, CTxOut& out2)
{
    vinCoralnode = vinCoralnodeIn;
    vchSig = vchSigIn;
...
std::string CObfuScationRelay::ToString()
{
    ...
    info << "vin: " << vinCoralnode.ToString() << " nBlockHeight: " << (int)nBlockHeight << " nRelayType: " << (int)nRelayType << " in " << in.ToString() << " out " << out.ToString();
...
{
    CCoralnode* pmn = mnodeman.GetCoralnodeByRank(nRank, nBlockHeight, ActiveProtocol());
