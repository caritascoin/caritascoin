...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "caritas" is a composite category enabling all CaritasCoin-related debug output
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\CARITAS
// Windows >= Vista: C:\Users\Username\AppData\Roaming\CARITAS
// Mac: ~/Library/Application Support/CARITAS
// Unix: ~/.caritas
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CARITAS";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "caritas.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty caritas.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override caritas.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "caritasd.pid"));
