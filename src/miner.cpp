...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
    ...
    if (!pblocktemplate.get()) {
        LogPrintf("Error getting block template..\n");
        return NULL;
    }
    ...
    if (bMasterNodePayment)
        LogPrintf("Masternode Payment enabled.\n");
...

    {
        ...
        while (!vecPriority.empty()) {
            ...
            // double check that there are no double spent zCARITAS spends in this block or tx
            if (tx.IsZerocoinSpend()) {
                int nHeightTx = 0;
                ...
                //This zCARITAS serial has already been included in the block, do not add this tx.
                if (fDoubleSerial)
                    continue;
            }
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("CaritasCoinMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("CaritasCoinMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("CaritasCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("caritas-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running CaritasCoinMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
